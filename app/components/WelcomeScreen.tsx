...
export default function WelcomeScreen({ onSuggestionClick, modelName }: WelcomeScreenProps) {
  return (
    <div className="flex flex-col items-center justify-center flex-1 px-4 py-16 animate-fadeIn">
      {/* Glow */}
      <div className="absolute w-72 h-72 bg-blue-600/8 rounded-full blur-[120px] pointer-events-none" />

      {/* Agent Icon */}
      <div className="relative mb-8">
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500/15 to-cyan-500/15 border border-white/[0.06] flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 w-full max-w-lg">
        {suggestions.map((s, i) => (
          <button
            ...
            className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.02] border border-white/[0.06] text-left text-sm text-zinc-400 hover:bg-white/[0.05] hover:border-blue-500/20 hover:text-zinc-200 transition-all duration-200 group cursor-pointer"
          >
            <span className="text-zinc-500 group-hover:text-blue-400 transition-colors">{s.icon}</span>
            <span>{s.text}</span>
          </button>
        ))}
      </div>
